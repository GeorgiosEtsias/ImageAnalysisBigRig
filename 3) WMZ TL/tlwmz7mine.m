function [tl,wmz,npoints,c25,c50,c75] = tlwmz7mine(xmat,ymat,zmat,cdata,edge,offset,tl_predicted)
% =========================================================================
...
if sizeC1(1,2) == 0 ||... % ie. no contours exist, or;
    ...
    for i = 1:length(ndatacontours)
        ...
        if minspany > max(max(ymat))/20 ||  minspanx >max(max(xmat))/50 
           
            % if the minimum y co-ord is greater than 5% above
            % the domain bottom, it probably isn't the correct contour
            % if the minimum x co-ord is greater than 2% of the total
            % distance from the x=0m boundary, it prob isn't correct
            spanx(i,1) = 0;
            spanx(i,2) = 0;
        else
            ...
            %     spany(i,1) = maxspany - minspany; % span of contour in y direction
            %     spany(i,2) = ndatacontours(1,i);
        end
